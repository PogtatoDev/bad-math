...
namespace General
{
		...
		{
				if (n < 20) return constants::FACTORIAL[n];

				real prod = 1;
				for (int k = 1; k <= n; k++) { prod *= k; }
		...
		bool is_int(real x)
		{
				if (std::abs(x - round(x)) < const_limits::LIM_EPS) return true;
				return false;
		}
		...
		real int_pow(real x, int n)
		{
				...
				if (is_even(n)) return int_pow(x * x, n / 2);
